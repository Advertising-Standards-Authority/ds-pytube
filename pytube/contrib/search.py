...
class Search:
    ...
    def append_video(vid_renderer, vid_metadata, videos=None):
        ...
        if vid_metadata["ad"]:
            resp = requests.get(vid_metadata["landing_page"])
            vid.author = resp.url.split("?")[0]
            vid.title = vid_metadata["title"]
            vid.ad = vid_metadata["ad"]
        else:
            try:
                vid.author = vid_metadata["channel_name"]
            except:
                logging.error(vid_metadata)
                raise Exception
            vid.title = vid_metadata["title"]
            vid.ad = vid_metadata["ad"]
        videos.append(vid)
                ...
                if "reelShelfRenderer" in video_details:
                    ...
                    for reelItemDict in video_details["reelShelfRenderer"]["items"]:
                        try:
                            ...
                            logging.error(err)
                            logging.error(reelItemDict)
                            raise KeyError
                        try:
                            vid_channel_data = vid_renderer["navigationEndpoint"][
                                "reelWatchEndpoint"
                            ...
                            vid_channel_uri = vid_channel_data["navigationEndpoint"][
                                "browseEndpoint"
                            ]["canonicalBaseUrl"]
                        except:
                            vid_channel_name = ""
                    ...
                    self.append_video(vid_renderer, vid_metadata, videos)
                elif "adSlotRenderer" in video_details:
                    vid_ad = True
                    try:
                        vid_renderer = video_details["adSlotRenderer"]
                    except:
                        logging.error(video_details)
                        raise KeyError
                    vid_id = vid_renderer["adSlotMetadata"]["slotId"]
                    vid_url = f"https://www.youtube.com/watch?v={vid_id}"
                    vid_title = vid_renderer["fulfillmentContent"]["fulfilledLayout"][
                        "inFeedAdLayoutRenderer"
                    ]["renderingContent"]["promotedSparklesWebRenderer"]["title"][
                        "simpleText"
                    ]
                    vid_click_url = vid_renderer["fulfillmentContent"][
                        "fulfilledLayout"
